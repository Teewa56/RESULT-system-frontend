            ...
            {(current === 1 || current === 2) && (
                <div className="flex flex-col w-full p-6">
                    <div className="relative w-full flex flex-col items-start justify-start">
                        ...
                        />
                        <div className={`absolute top-12 rounded-2xl bg-gray-300 w-full flex flex-col items-start justify-start ${searchResult && searchResult.length > 0 ? 'p-3' : 'p-0'} transition-all duration-300 overflow-hidden`}>
                        {searchResult && searchResult.length > 0 && (
                            searchResult.map((result) => (
                                <Link 
                                    to={`/admin/profile/${result._id}`}
                                    key={result._id}
                                    className="flex items-center justify-start rounded-3xl w-full border-black my-2 bg-gray-500">
